...
// struct ExtendAttrs {
//     #[darling(default)]
//     x_priority: Option<i32>, // 使用Option表示可能不存在
// }
...
pub fn derive_x_priority(input: TokenStream) -> TokenStream {
    ...
    for field in fields.named.iter() {
        debug_file
            .write_fmt(format_args!("Processing field: {:?}\n", field.ident))
            .unwrap();
        ...
        let priority = field
            ...
            .filter_map(|attr| {
                ...
                attr.parse_args_with(
                    |input: syn::parse::ParseStream| -> syn::Result<Option<i64>> {
                        ...
                        while !input.is_empty() {
                            ...
                            if path.is_ident("extend") {
                                if input.peek(syn::token::Paren) {
                                    ...
                                    debug_file
                                        .write_all("Found extend attribute\n".as_bytes())
                                        .unwrap();

                                    while !content.is_empty() {
                                        // 检查是否是键值对
                                        if content.peek(syn::LitStr) || content.peek(syn::Ident) {
    ...
    // 构建输出 tokens
    let output_tokens = output.iter().map(|(name, pri)| {
        quote! { (#name.to_string(), #pri) }
    });

    quote! {
        impl #struct_name {
            pub fn x_priorities() -> Vec<(String, i64)> {
                let mut ordered_keys = vec![
                    #(#output_tokens),*
                ];

                ordered_keys.sort_by(|a, b| b.1.cmp(&a.1));

                ordered_keys
            }

            pub fn x_priorities_json() -> &'static str {
                #output_json
            }
        }
    }
